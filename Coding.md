+ 管道和进程间通信，即`pipe()`+`fork()`的形式，注意关掉无用的文件文件描述符。先pipe()，之后先把**能立刻用完****之后就不在用的符**立刻用完，然后将他们关掉，再fork()。因为子进程会继承父进程的文件描述符。例：[MIT6.S081 lab的第三个求2-35内的素数](http://xv6.dgs.zone/labs/requirements/lab1.html)。
```c
#include <stidio.h>
